# ЗАКОН АЛЛАХА
- НЕ ПУШИТЬ ГОВНО В main
- СПИСОК ХАРАМА (НЕ ПУШИТЬ НИКУДА): .idea venv "__pycache__" .pyc
...
# Инструкция по Джихаду:
ВОЙТИ В МЕЧЕТЬ
git init
git config --global user.email example@allah.com
ИЛИ git config --local user.email example@allah.com

ЗАМОЛИТЬ ИЗМЕНЕНИЯ ПЕРЕД АЛЛАХОМ
git add --all
